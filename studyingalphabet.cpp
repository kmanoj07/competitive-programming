...
{
    string s;cin>>s;
    //Create an array to store the 26 value for corresponding alphabets and store 0 at every block 
    int a[26] = {0};
    //this is the list of char in the string jeff has
    for(int i =0; i < s.length(); i++)
    {
        char c = s[i];
        int n = (int)c; // GET ASCII CODE FOR THIS CHAR
        int index = n - 97; //Get the index for the block which represent the char
        a[index] = 1; // set the value for that index block 1 
    }
    int n;cin>>n;
    while(n--)
    {
